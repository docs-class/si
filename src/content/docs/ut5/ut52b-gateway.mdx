...
#### Ejemplo de petición ARP

Supón que el PC (`192.168.1.10`) quiere comunicarse con la impresora (`192.168.1.50`). No conoce su dirección MAC, así que lanza una petición ARP en broadcast:

<Steps>

1. **ARP Request** — el PC pregunta a toda la red:
   ```
   ¿Quién tiene la IP 192.168.1.50? Contacte con 192.168.1.10
   Origen:  IP 192.168.1.10  |  MAC aa:bb:cc:11:22:33
   Destino: IP 192.168.1.50  |  MAC ff:ff:ff:ff:ff:ff  ← broadcast
   ```

2. **ARP Reply** — solo la impresora responde, en unicast:
   ```
   192.168.1.50 está en MAC dd:ee:ff:44:55:66
   Origen:  IP 192.168.1.50  |  MAC dd:ee:ff:44:55:66
   Destino: IP 192.168.1.10  |  MAC aa:bb:cc:11:22:33
   ```

3. El PC guarda la entrada en su **tabla ARP** (caché) para no repetir la solicitud:
   ```
   192.168.1.50  →  dd:ee:ff:44:55:66
   ```

</Steps>

:::note[Destino externo]
Si el PC quiere llegar a `93.184.216.34` (externo), el paso 1 cambia: en lugar de pedir la MAC de `93.184.216.34`, pregunta por la MAC del **router** (`192.168.1.1`), ya que es el que debe reenviar el tráfico fuera de la red local.
:::
